...
def _expected_energy_from_curve(
    ws: np.ndarray,
    ...
    *,
    ...
    - left=0 below cut-in/curve support
    - right=plateau at max curve power above curve support (until cut-out)
    - enforce cut-out: ws >= cut_out_ws -> 0
    """
...
def build_ml_datasets_per_turbine(
    *,
    ...
    max_ws_fraction_of_rated: float = 1.00,
    ...
    def _preprocess_rename_time(ds: xr.Dataset) -> xr.Dataset:
        if "valid_time" in ds.dims or "valid_time" in ds.coords:
            ds = ds.rename({"valid_time": "time"})
        return ds

    ds_main = xr.open_mfdataset(
        _to_path_list(era5_main_paths),
        combine="by_coords",
        join="outer",
        compat="no_conflicts",
        preprocess=_preprocess_rename_time,
        coords="minimal", data_vars="minimal"
    )

    ds_fsr_zust = xr.open_mfdataset(
        _to_path_list(era5_fsr_zust_paths),
        combine="by_coords",
        join="outer",
        compat="no_conflicts",
        preprocess=_preprocess_rename_time,
    ...
    for turbine_id, df in scada.groupby("turbine_id"):
        ...
        cut_out = float(turbine_curves[turbine_type].get("Cut-out wind speed", np.inf))
        ...
        ds_r = ds_r.reindex(time=ds_m["time"])
        ds_r = ds_r[["fsr", "zust"]].load()

        for v in ["zust", "fsr"]:
            if v in ds_r:
                ds_r[v] = ds_r[v].interpolate_na(dim="time", method="linear")
                ds_r[v] = ds_r[v].ffill("time").bfill("time")
        ...
        merged["target_is_valid"] = (
            ...
            & (merged["energy_kwh"] <= 1.05 * merged["rated_power_kw"])
        )
        # --- optional: require availability if present
        if "is_available" in merged.columns:
            merged["target_is_valid"] = merged["target_is_valid"] & merged["is_available"].astype(bool)

        # --- compute correction factor candidates for robust filtering
        exp_e = merged["expected_energy_kwh"].astype(float)
        ok_exp = np.isfinite(exp_e) & (exp_e > 0.0)

        cf = pd.Series(np.nan, index=merged.index, dtype=float)
        cf.loc[ok_exp] = merged.loc[ok_exp, "energy_kwh"].astype(float) / exp_e.loc[ok_exp]

        logcf = np.log(cf.where(cf > 0))
