...
def validate_ml_dataset(
    ...
    *,
    ...
    & (df["energy_kwh"] > 1.05 * df["rated_power_kw"])
    )
    
    if verbose:
        frac = float(over.mean())
        if frac > 0:
            print(f"⚠️ WARNING: Observed energy exceeds rated power by >5% (share={100*frac:.3f}%)")
            print(df.loc[over, ["turbine_id","timestamp","energy_kwh","rated_power_kw"]]
                  .sort_values("energy_kwh", ascending=False)
                  .head(10))
        else:
            print("✓ No observed energy > 105% of rated power (row-wise)")
