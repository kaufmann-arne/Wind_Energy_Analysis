...
def preprocess_scada_dataset(
    ...
    *,
    ...
    for i, file in enumerate(files, start=1):
        ...
        # Ensure is_available exists for hourly data too (simple heuristic)
        if "is_available" not in df.columns:
            if is_hourly:
                ws = df["wind_speed_ms"]
                # available unless: wind >= cut-in but energy is ~0 (likely downtime/curtailment)
                df["is_available"] = ~((ws >= cut_in_ws) & (df["energy_kwh"] <= 0))
            else:
                # 10-min case is handled below
        ...
        if qc_col is not None:
            ...
            before = len(df)
            qc = df[qc_col]
            qc_good = qc.isin([1, True, "1", "True", "true", "TRUE", "yes", "YES", "Y", "y"])
            df = df[qc_good].copy()
            after = len(df)
            df["is_available"] = True
        ...
        if is_10min:
            ...
            def hourly_aggregate(g: pd.DataFrame):
                    if g.empty or "is_available" not in g.columns:
                        return None
                
                    n_valid = g["power_kw"].count()
                    if n_valid < 4:
                        return None
                    ...
                    energy_kwh = g["power_kw"].sum(min_count=1) / 6.0
